...
{ readonly LiveDataStore _store;readonly LiveChartDataService _charts;readonly DetailNavigationService _nav;readonly LineSeries<LiveChartPoint> _line;
 ...
 public LiveAnomalyResultDto? AiAnalysis=>Pump?.AiAnalysis;
 public void Select(int id){PumpId=id;Update();} partial void OnMetricChanged(string value)=>Update(); void Update(){if(PumpId is int id)_line.Values=_charts.GetSeries(LiveChartDataService.GetPumpMetricKey(id,Metric));_line.Name=Metric;YAxes[0].Name=Metric;OnPropertyChanged(nameof(Pump));OnPropertyChanged(nameof(AiAnalysis));}void Changed(object?s,NotifyCollectionChangedEventArgs e){OnPropertyChanged(nameof(Pump));OnPropertyChanged(nameof(AiAnalysis));}[RelayCommand]void Back()=>_nav.BackToPumps(); }
