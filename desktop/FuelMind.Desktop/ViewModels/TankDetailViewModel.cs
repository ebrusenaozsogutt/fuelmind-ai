...
public sealed partial class TankDetailViewModel : ObservableObject
{
 ...
 public LiveAnomalyResultDto? AiAnalysis=>Tank?.AiAnalysis;
 public void Select(int id){TankId=id;_line.Values=_charts.GetSeries(LiveChartDataService.GetMeasuredTankLevelMetricKey(id));Notify();} void Changed(object? s,NotifyCollectionChangedEventArgs e)=>Notify(); void Notify(){OnPropertyChanged(nameof(Tank));OnPropertyChanged(nameof(Fill));OnPropertyChanged(nameof(AiAnalysis));}
